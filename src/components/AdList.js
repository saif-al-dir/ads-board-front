...
const AdList = () => {
  ...
  useEffect(() => {
    const fetchAds = async () => {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/ads`);
      ...
      {ads.map(ad => (
        <Col key={ad._id} className="d-flex">
          <Card className="flex-fill">
            {ad.image && (
              <Card.Img variant="top" src={`${process.env.REACT_APP_API_URL}/uploads/${ad.image}`} alt={ad.title} />
            )}
            <Card.Body>
